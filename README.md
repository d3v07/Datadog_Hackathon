...
The SSE slice implements `GET /v1/stream?token=demo_token_acme_corp_2026` with org-scoped delivery, 15-second `:heartbeat` keepalives, stable event ids, and `Last-Event-ID` replay for retained in-memory events.
