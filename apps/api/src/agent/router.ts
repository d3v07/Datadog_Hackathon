...
function createPlainTextBody(input: RouteChangeReportInput): string {
  const headline = input.changeReport.headline ?? input.changeReport.recommendation.copy;
  const change = input.changeReport.changes[0]?.summary ?? headline;
  const citation = input.changeReport.citations?.[0] ?? input.changeReport.changes[0]?.citations?.[0];
  const citationText = citation ? ` Citation: ${citation.label ?? citation.section ?? "Source"} (${citation.sourceUrl ?? citation.url}).` : "";
